...
function enviaAlert() {
  swal({
      title: '',
      text: 'Confirma a atualização da revisão do relatório?',
      ...
      if (isConfirm) {
        //$("#FormAltera");
        $('#FormAltera').attr('action', '/fARevisarRVBaseAtualiza').submit();
      }
  })
}
...
function enviaAlert1() {
  swal({
      title: '',
      text: 'Confirma a revisão do relatório?',
      type: 'warning',                  //"warning", "error", "success", "info"
      showCancelButton: true,
      confirmButtonColor: '#3085d6',
      cancelButtonColor: '#D80C1B',
      ...
      if (isConfirm) {
        //$("#FormAltera").submit();
        $('#FormAltera').attr('action', '/fARevisarRVBaseConclui').submit();
      }
  })
}

function enviaAlert2($dt_acomp) {
  swal({
      title: '',
